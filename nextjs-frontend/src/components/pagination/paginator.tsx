...
const baseClassName: string = (<a className="inline-flex h-8 w-8 items-center justify-center font-bold" />).props
	.className;
...
export function Paginator({
	...
	const [bookendStart, preEllipsis, prePage, pageMarker, postPage, postEllipsis, bookendEnd] = paginator.pagination(
		page,
		pageCount
	);
	...
	return (
		<div className="flex gap-2">
			...
			{pages(bookendStart)}
			{preEllipsis === '...' ? <PageDisplay>&hellip;</PageDisplay> : pages(preEllipsis)}
			{pages(prePage)}
			<PageDisplay>{pageMarker}</PageDisplay>
			{pages(postPage)}
			{postEllipsis === '...' ? <PageDisplay>&hellip;</PageDisplay> : pages(postEllipsis)}
			{pages(bookendEnd)}
			{page === pageCount ? (
				<>
					<PageDisplay>
