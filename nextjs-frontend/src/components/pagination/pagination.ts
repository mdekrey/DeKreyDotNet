...
export function createPaginator(
	...
	function boundaries(currentPage: number, lastPage: number) {
		...
		const preEllipsisStart = Math.max(1, Math.min(startBookendEnd, aroundStart, endFocusStart));
		...
		const postPageEnd = Math.min(Math.max(startFocusEnd, aroundEnd), lastPage + 1);
		const postEllipsisStart = postPageEnd;
		const postEllipsisEnd = Math.min(lastPage + 1, Math.max(postPageEnd, aroundEnd, endBookendStart));
