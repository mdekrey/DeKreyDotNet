...
> extends THREE.Object3D {
	...
	private readonly particleStructure: Readonly<
		Record<string, ParticleStructureEntry>
	>;
	...
	constructor(options: ParticleSystemOptions<TStructureKey | 'isActive'>) {
		...
		this.particleStructure = this.options.particleStructure.reduce<
			Record<string, ParticleStructureEntry>
	...
	toBufferIndex(index: number, structureKey: TStructureKey | 'isActive') {
		return (
			// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
			index * this.offsetSize + this.particleStructure[structureKey]!.offset
		);
	}
}
