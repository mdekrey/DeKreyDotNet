...
Currently these workflows are written to leverage the [Scalable Git Branching
Tools][scalable-git-branching-tools], though the naming conventions match the
old style. I'll update this article with an update to the tool that uses the new
terminology... hopefully soon.
...
* Create a new feature or integration branch with other dependencies:
    ...
    ```sh
    git new feature/new-feature -upstreamBranches branch1,branch2,branch3
    ```
...
* Combine branch depenencies, such as when a sub-feature has been merged into its parent
  feature
    ...
    ```sh
    git refactor-upstream -source feature/login-form-validation -target feature/login-page -combine
    ```
...
* Replace branch dependencies, such as after renaming a branch
    ...
    ```sh
    git refactor-upstream -source old-branch-name -target new-branch-name -rename
    ```
...
* List dependencies of a branch, recursively:
    ...
    ```sh
    git show-upstream branch-name -recurse
    ```
...
* List dependants of a branch, recursively:
    ...
    ```sh
    git show-downstream branch-name -recurse
    ```
...
* Create a release candidate from multiple dependencies:
    ...
    ```sh
    git rc rc/2024-11-01 -upstreamBranches branch1,branch2,branch3
    ```
...
* Remove a branch from dependency lists, redirecting all dependencies that were
  on it to the target.
    ...
    ```sh
    git refactor-upstream -source old-branch-name -target new-branch-name -remove
    ```
